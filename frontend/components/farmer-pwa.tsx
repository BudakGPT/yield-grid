...
export function FarmerPwa() {
  ...
  const [dominantLabel, dominantPercentage] = dominantGrade({ A: gradeA, B: gradeB, C: gradeC });
            ...
            <AnimatePresence mode="wait">
              {tab === "scan" && <motion.div key={`scan-${scanView}`} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} className="p-4">
                {scanView === "capture" && <>
                  ...
                  <label className="relative mt-5 block h-[285px] cursor-pointer overflow-hidden rounded-[1.5rem] border border-forest-950/10 bg-forest-950">{preview ? <Image src={preview} alt="Crate preview" fill unoptimized className="object-cover" /> : <div className="grid h-full place-items-center text-white/45"><Camera className="size-10" /></div>}<div className="absolute inset-0 bg-gradient-to-t from-forest-950/82 via-transparent to-transparent" /><input type="file" accept="image/*" capture="environment" onChange={(event) => onPhoto(event.target.files?.[0])} className="sr-only" /><span className="absolute bottom-4 left-4 inline-flex items-center gap-2 rounded-xl bg-white/92 px-3 py-2 text-[9px] font-extrabold text-forest-950"><Camera className="size-3.5" />{preview ? "Retake photo" : "Take a photo"}</span></label>
                  <label className="mt-3 flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl border border-forest-950/9 bg-white text-[9px] font-extrabold text-forest-950"><ImagePlus className="size-3.5" /> Choose from gallery instead<input type="file" accept="image/*" onChange={(event) => onPhoto(event.target.files?.[0])} className="sr-only" /></label>
                ...
                {scanView === "result" && grading && <>
                  ...
                  <div className="mt-3 rounded-[1.4rem] bg-forest-950 p-4 text-white"><div className="flex items-start justify-between"><div><p className="eyebrow !text-leaf-400">Dominant visual grade</p><h3 className="mt-2 text-3xl font-black">{dominantPercentage}% {dominantLabel}</h3></div><span className="pill bg-white/8 text-leaf-400"><Clock3 className="size-3" /> ~{grading.est_shelf_life.approx_days} days</span></div><div className="mt-5 flex h-2 overflow-hidden rounded-full bg-white/10"><i className="bg-leaf-400" style={{ width: `${gradeA}%` }} /><i className="bg-[#e6bb68]" style={{ width: `${gradeB}%` }} /><i className="bg-clay-500" style={{ width: `${gradeC}%` }} /></div><div className="mt-2 flex justify-between font-mono text-[7px] uppercase text-white/40"><span>A {gradeA}%</span><span>B {gradeB}%</span><span>C {gradeC}%</span></div><p className="mt-4 border-t border-white/8 pt-3 text-[8px] text-white/42">The largest share is highlighted automatically</p></div>
                  <div className="mt-3 grid grid-cols-3 gap-2">{grading.grade_recommendations.map((item) => <div key={item.grade} className={`rounded-xl p-3 ${item.grade === dominantLabel ? "bg-leaf-100 text-forest-700" : "bg-white text-ink-600"}`}><p className="text-[8px] font-black">Grade {item.grade}</p><p className="mt-1 text-[7px] font-bold leading-3">{item.title}</p></div>)}</div>
