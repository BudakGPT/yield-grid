        ...
        <div className="overflow-hidden rounded-[2.2rem] border-[7px] border-forest-950 bg-cream-50 shadow-[0_32px_90px_rgba(12,36,25,.22)]">
          ...
          <div className="relative min-h-[650px] bg-cream-50 pb-20">
            <AnimatePresence mode="wait">
              ...
              {tab === "profile" && <motion.div key="profile" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4"><div className="rounded-[1.5rem] bg-white p-5 text-center"><span className="mx-auto grid size-20 place-items-center rounded-full bg-[#efd090] text-2xl font-black">{session?.user.fullName.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span><h3 className="mt-4 text-xl font-black">{session?.user.fullName}</h3><p className="mt-1 text-[9px] text-ink-600">{session?.user.stellarPublicKey ?? "Stellar wallet pending"}</p><span className="pill mt-4 bg-leaf-100 text-forest-700"><BadgeCheck className="size-3" /> Authenticated farmer</span></div></motion.div>}
            </AnimatePresence>
