...
import { AMARA_LISTING, MARKETPLACE_LISTINGS, type BuyerSegment, type MarketplaceListing } from "@/lib/demo-data";
  ...
  return (
    <article className="panel group overflow-hidden p-2">
      <div className="relative h-60 overflow-hidden rounded-[1.15rem]">
        <Image src={listing.image} alt={listing.produce} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition duration-700 group-hover:scale-105" />
        <div className="absolute inset-0 bg-gradient-to-t from-forest-950/82 via-transparent to-transparent" />
        <div className="absolute left-3 top-3 flex gap-2">{listing.fresh && <span className="pill bg-leaf-400 text-forest-950"><RadioTower className="size-3" /> Just listed</span>}<span className="pill bg-white/90 text-forest-950"><Clock3 className="size-3" /> {listing.shelfLife}</span></div>
...
export function BuyerMarketplace() {
  ...
  const listings = useMemo(() => {
    const all = state.listingLive ? [AMARA_LISTING, ...MARKETPLACE_LISTINGS] : MARKETPLACE_LISTINGS;
    return segment === "all" ? all : all.filter((listing) => listing.segment === segment);
  }, [segment, state.listingLive]);

  const confirmEscrow = () => {
    setLocking(true);
    window.setTimeout(() => {
      lockEscrow(selected?.id);
      setLocking(false);
      setSelected(null);
      router.push("/order");
    }, 1400);
    ...
    <>
      <div className="grid gap-4 xl:grid-cols-[270px_minmax(0,1fr)]">
        <aside className="panel h-fit p-3 xl:sticky xl:top-28">
          <div className="grid-field rounded-2xl p-5 text-white"><span className="grid size-10 place-items-center rounded-xl bg-leaf-400 text-forest-950"><ChefHat className="size-4" /></span><p className="eyebrow mt-7 !text-leaf-400">Buyer persona</p><h2 className="mt-2 text-xl font-black">Chef Rosa</h2><p className="mt-2 text-[10px] leading-5 text-white/45">Commercial kitchen · Jakarta</p><div className="mt-6 rounded-xl bg-white/7 p-3"><p className="text-[8px] uppercase tracking-wider text-white/38">Looking for</p><p className="mt-2 text-[10px] font-bold">Grade A · ≥5 day shelf life · 30–100 kg</p></div></div>
          ...
          <div className="mt-4 rounded-xl border border-clay-500/15 bg-clay-100 p-3 text-[8px] leading-4 text-clay-500"><strong>Frontend demo.</strong> Listings, reputation, and escrow states are dummy data shaped to the PRD contracts.</div>
        </aside>

        <section>
          <div className="flex flex-col gap-4 rounded-[1.5rem] bg-[#dfe79a] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><span className="eyebrow">Live marketplace feed</span><h2 className="mt-2 text-2xl font-black tracking-[-.04em] text-forest-950">Verified before you buy.</h2><p className="mt-2 text-[10px] text-forest-950/55">Quality distribution, shelf-life band, Codex mapping, and direct farm price.</p></div><span className="pill w-fit bg-forest-950 text-white"><RadioTower className="size-3 text-leaf-400" /> {state.listingLive ? "New listing received" : "Waiting for Amara’s scan"}</span></div>

          <div className="mt-4 grid gap-4 md:grid-cols-2 2xl:grid-cols-3">{listings.map((listing) => <ListingCard key={listing.id} listing={listing} onBuy={() => setSelected(listing)} orderActive={listing.id === AMARA_LISTING.id && state.status !== "open"} />)}</div>
          {listings.length === 0 && <div className="panel mt-4 grid min-h-72 place-items-center text-center"><div><Leaf className="mx-auto size-8 text-ink-600/30" /><h3 className="mt-4 text-lg font-black">No match in this segment yet.</h3><p className="mt-2 text-[10px] text-ink-600">Run Amara’s scan or switch buyer segment.</p></div></div>}
        </section>
      </div>

      <AnimatePresence>{selected && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)} className="fixed inset-0 z-[80] grid place-items-end bg-forest-950/55 p-3 backdrop-blur-sm sm:place-items-center"><motion.div initial={{ y: 30, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, opacity: 0 }} className="w-full max-w-lg overflow-hidden rounded-[1.8rem] bg-white shadow-2xl"><div className="flex items-start justify-between bg-forest-950 p-5 text-white"><div><p className="eyebrow !text-leaf-400">Direct order</p><h2 className="mt-2 text-2xl font-black">Buy from {selected.farmer}</h2><p className="mt-1 text-[9px] text-white/42">No quality-and-trust broker markup</p></div><button onClick={() => setSelected(null)} className="grid size-10 place-items-center rounded-xl bg-white/8" aria-label="Close"><X className="size-4" /></button></div><div className="p-5"><div className="flex items-center gap-3 rounded-2xl bg-cream-50 p-3"><div className="relative size-16 overflow-hidden rounded-xl"><Image src={selected.image} alt="" fill className="object-cover" /></div><div className="min-w-0 flex-1"><p className="text-xs font-black">{selected.produce} · {selected.weightKg} kg</p><p className="mt-1 text-[9px] text-ink-600">{selected.grade.A}% A · {selected.shelfLife}</p></div><strong className="text-sm">Rp{(selected.weightKg * selected.pricePerKg).toLocaleString("id-ID")}</strong></div><div className="mt-4 space-y-2">{[[ShieldCheck, "Quality visible", selected.codex], [LockKeyhole, "Funds secured", "Held in Stellar testnet escrow · demo"], [Leaf, "Farmer receives", "Released after delivery verification"]].map(([Icon, title, copy]) => { const ItemIcon = Icon as typeof ShieldCheck; return <div key={String(title)} className="flex gap-3 rounded-xl border border-forest-950/8 p-3"><ItemIcon className="mt-0.5 size-4 shrink-0 text-forest-700" /><div><p className="text-[9px] font-black">{String(title)}</p><p className="mt-1 text-[8px] text-ink-600">{String(copy)}</p></div></div>; })}</div><button onClick={confirmEscrow} disabled={locking} className="mt-5 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-leaf-400 text-[10px] font-black text-forest-950 disabled:opacity-70">{locking ? <><motion.i animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1 }} className="size-4 rounded-full border-2 border-forest-950/25 border-t-forest-950" />Locking demo escrow...</> : <><LockKeyhole className="size-4" />Accept & lock escrow</>}</button><p className="mt-3 text-center text-[8px] text-ink-600">Frontend simulation · no real funds move</p></div></motion.div></motion.div>}</AnimatePresence>
    </>
  );
}
