    ...
    <div className={`grid gap-4 ${compact ? "" : "lg:grid-cols-[.72fr_1.28fr]"}`}>
      <aside className="rounded-[1.5rem] bg-forest-950 p-5 text-white">
        ...
        <div className="mt-6 space-y-2 text-[9px]">
          ...
          <div className="flex items-center gap-2 rounded-xl bg-white/6 p-3"><WalletCards className="size-4 text-leaf-400" />{profile?.walletReady ? "Stellar wallet ready" : "Stellar wallet pending"}</div>
          <div className="flex items-center gap-2 rounded-xl bg-white/6 p-3"><ShieldCheck className="size-4 text-leaf-400" />Email and role are managed securely by Supabase</div>
        </div>
      </aside>

      <form onSubmit={submit} className="rounded-[1.5rem] bg-white p-5">
        <div className="flex items-start justify-between gap-3"><div><p className="eyebrow">Account profile</p><h2 className="mt-2 text-xl font-black">Your YieldGrid identity.</h2></div><UserRound className="size-5 text-forest-700" /></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-[9px] font-bold text-ink-600">Phone number<input type="tel" maxLength={32} value={form.phoneNumber} onChange={(event) => change("phoneNumber", event.target.value)} placeholder="+62 812 3456 7890" className="mt-2 min-h-11 w-full rounded-xl bg-cream-100 px-3 text-[10px] text-forest-950 outline-none ring-leaf-400 focus:ring-2" /></label>
          <label className="text-[9px] font-bold text-ink-600 sm:col-span-2">Location<span className="relative mt-2 block"><MapPin className="absolute left-3 top-3.5 size-3.5 text-ink-600/45" /><input maxLength={120} value={form.location} onChange={(event) => change("location", event.target.value)} placeholder="Bogor, West Java" className="min-h-11 w-full rounded-xl bg-cream-100 pl-9 pr-3 text-[10px] text-forest-950 outline-none ring-leaf-400 focus:ring-2" /></span></label>
          <label className="text-[9px] font-bold text-ink-600 sm:col-span-2">Bio<textarea maxLength={500} rows={4} value={form.bio} onChange={(event) => change("bio", event.target.value)} placeholder="Tell buyers or farmers about your work." className="mt-2 w-full resize-none rounded-xl bg-cream-100 p-3 text-[10px] leading-5 text-forest-950 outline-none ring-leaf-400 focus:ring-2" /><span className="mt-1 block text-right font-mono text-[7px] text-ink-600/45">{form.bio.length}/500</span></label>
          <label className="text-[9px] font-bold text-ink-600 sm:col-span-2">Avatar URL<input type="url" maxLength={2048} value={form.avatarUrl} onChange={(event) => change("avatarUrl", event.target.value)} placeholder="https://..." className="mt-2 min-h-11 w-full rounded-xl bg-cream-100 px-3 text-[10px] text-forest-950 outline-none ring-leaf-400 focus:ring-2" /></label>
        ...
        {saved && <p role="status" className="mt-4 flex items-center gap-2 rounded-xl bg-leaf-100 p-3 text-[9px] font-bold text-forest-700"><BadgeCheck className="size-4" />Profile saved and session updated.</p>}
